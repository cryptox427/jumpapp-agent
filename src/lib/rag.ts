...
export class RAGService {
  ...
  async searchEmails(query: string, limit: number = 5, googleService?: any) {
    try {
  ...
  private async searchEmailsFromGmail(query: string, limit: number, googleService: any) {
    try {
      const gmail = await googleService.getGmailService()
      
      // Convert natural language query to Gmail search syntax
      const gmailQuery = this.convertToGmailQuery(query)
  ...
  async searchAll(query: string, limit: number = 10, googleService?: any) {
    try {
  ...
  async getHubSpotContext(query: string, limit: number = 10) {
    try {
      ...
      const context = {
        contacts: contacts,
        notes: notes,
        summary: this.formatHubSpotContextSummary(contacts, notes),
      }
  ...
  async getContextForQuery(query: string, limit: number = 10, googleService?: any) {
  ...
  private formatHubSpotContextSummary(contacts: any[], notes: any[]): string {
  ...
  private formatContextSummary(results: any[]): string {
    if (results.length === 0) {
